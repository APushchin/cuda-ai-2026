...
std::vector<float> GeluCUDA(const std::vector<float>& input)
{
    ...
    float* host_input_buffer  = nullptr;
    error = cudaMalloc(&host_input_buffer, size_in_bytes);
    if (error != cudaSuccess) {
        cudaStreamDestroy(stream);
        throw std::runtime_error("Failed to allocate device memory for input");
    }

    float* host_output_buffer = nullptr;
    error = cudaMalloc(&host_output_buffer, size_in_bytes);
    if (error != cudaSuccess) {
        cudaFree(host_input_buffer);
        cudaStreamDestroy(stream);
    ...
    if (error != cudaSuccess) {
        cudaFree(host_input_buffer);
        cudaFree(host_output_buffer);
        cudaStreamDestroy(stream);
        throw std::runtime_error("Failed to copy data to device");
    }

    float* host_output_buffer_pinned = nullptr;
    error = cudaMallocHost(&host_output_buffer_pinned, size_in_bytes);
    if (error != cudaSuccess) {
        cudaFree(host_input_buffer);
        cudaFree(host_output_buffer);
        cudaStreamDestroy(stream);
    ...
    if (error != cudaSuccess) {
        cudaFree(host_input_buffer);
        cudaFree(host_output_buffer);
        cudaFreeHost(host_output_buffer_pinned);
        cudaStreamDestroy(stream);
    ...
    std::memcpy(result.data(), host_output_buffer_pinned, size_in_bytes);
